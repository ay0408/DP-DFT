...
・Evaluating the relationship among elements of the reconstructed vector in DFT rigorously. (The assumption in this study that "the original $k$-th element $g_k$ cannot be recovered from the reconstructed $k'(\neq k)$-th element $\hat{g}_{k'}$" is too rough. More precisely, we should assume "the sorts in neighboting datasets are indistinguishable using the output elements". (Can we really say that this assumption is reasonable?)) <---- I plan to work on this problem and improve our algorithms in the near future.
