...
・Evaluating the relationship among elements of the reconstructed vector in DFT rigorously. (The assumption in this study that "the original $k$-th element $g_k$ cannot be recovered from the reconstructed $k'(\neq k)$-th element $\hat{g}_{k'}$" is too rough. (Can we really say that the sorts in neighboring datasets are indistinguishable from the $k$ output elements?)) <---- I plan to work on this problem and improve our algorithms in the near future.
